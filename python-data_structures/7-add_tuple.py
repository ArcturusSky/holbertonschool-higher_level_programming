...
def add_tuple(tuple_a=(), tuple_b=()):
    ...
    if len(tuple_a) - 1 < 2:                # Check if enough item  in tuple
        my_lista = list(tuple_a)        # Convert to list to be mutable

        while len(my_lista) - 1 < 2:        # Append 0 until correct len
            my_lista.append(0)

        tuple_a = tuple([my_lista[0], my_lista[1]])     # Convert back to tuple

    if len(tuple_b) - 1 < 2:                # Same for tuple_b
        my_listb = list(tuple_b)

        while len(my_listb) - 1 < 2:
            my_listb.append(0)
